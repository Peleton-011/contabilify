...
export function useLedgers() {
  ...
  const { ledgerActivoId, seleccionar, cargarDesdeStorage } = useLedgerActivo()
  ...
  async function fetchLedgers() {
    ...
    if (err) {
      ...
      const filas = (data ?? []) as unknown as Array<{ role: Rol; ledgers: Ledger | null }>
      ledgers.value = filas
        .filter((fila): fila is { role: Rol; ledgers: Ledger } => !!fila.ledgers)
  ...
  async function crearLedger(nombre: string) {
    const { data, error: err } = await supabase.rpc('crear_ledger', { p_nombre: nombre })
    if (err) throw err
    await fetchLedgers()
  ...
  function asegurarLedgerActivo() {
    cargarDesdeStorage()
    if (ledgerActivoId.value && ledgers.value.some((l) => l.id === ledgerActivoId.value)) return
    if (ledgers.value.length) seleccionar(ledgers.value[0].id)
  }

  const ledgerActivo = computed(() => ledgers.value.find((l) => l.id === ledgerActivoId.value) ?? null)
